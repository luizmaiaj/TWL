...
bool Engine::detectCollisions(PlayableCharacter& character)
{
	...
	FloatRect level(0, 0, m_LM.getLevelSize().x * (float)TILE_SIZE, m_LM.getLevelSize().y * (float)TILE_SIZE);
	if (!character.getPosition().intersects(level))
	{
		// respawn the character
		character.spawn(m_LM.getStartPosition(), (float)GRAVITY);
	}
	...
	for (int x = startX; x < endX; x++)
	{
		for (int y = startY; y < endY; y++)
		{
			...
			block.left = x * (float)TILE_SIZE;
			block.top = y * (float)TILE_SIZE;
