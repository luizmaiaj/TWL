			...
			if (arrayLevel[y][x] == 2)// fire is present
			{
					...
					(float)TILE_SIZE).intersects(previousEmitter))
				{
					...
					vSoundEmitters.push_back(
						Vector2f((float)x * TILE_SIZE, (float)y * TILE_SIZE));

					// Make a rectangle 6 blocks x 6 blocks,
					// so we don't make any more emitters 
					// too close to this one
					previousEmitter.left = (float)x * TILE_SIZE;
					previousEmitter.top = (float)y * TILE_SIZE;
					previousEmitter.width = (float)TILE_SIZE * 6;
					previousEmitter.height = (float)TILE_SIZE * 6;
				}
